...
rule all:
    input:
        ...
        minia_assembly_gfa =  expand(join_path('results', 'test_abundance', 'minia', 'A{abundance}.K{kmer}', 'minia.assembly.contigs.gfa'), abundance=list(range(3,15)), kmer=list(range(21,31))),
        # # SINGLE ASSEMBLY
        # samples = expand(join_path(config['data']['reads'], '{sample}.merged.fastq'), sample=SAMPLES),
